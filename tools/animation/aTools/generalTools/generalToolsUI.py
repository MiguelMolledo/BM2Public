...
from BM2Public.tools.animation.aTools.commonMods      import uiMod
from BM2Public.tools.animation.aTools.commonMods      import utilMod
from BM2Public.tools.animation.aTools.commonMods      import animMod
from BM2Public.tools.animation.aTools.commonMods      import commandsMod
from BM2Public.tools.animation.aTools.commonMods      import aToolsMod
from BM2Public.tools.animation.aTools                 import setup
from BM2Public.tools.animation.aTools.generalTools    import hotkeys;                     reload(hotkeys)
from BM2Public.tools.animation.aTools.generalTools    import tumbleOnObjects;             reload(tumbleOnObjects)
from BM2Public.tools.animation.aTools.animTools       import animationCrashRecovery;      reload(animationCrashRecovery)
from BM2Public.tools.animation.aTools.animTools       import framePlaybackRange;          reload(framePlaybackRange)
from BM2Public.tools.animation.aTools.animTools       import jumpToSelectedKey;           reload(jumpToSelectedKey)
...
def shelfButton(*args):
    ...
    command         = "from BM2Public.tools.animation.aTools.animTools.animBar import animBarUI; animBarUI.show('toggle')"
...
"""
def launchManipOrient():
    #scriptJob
    manipOrientScriptJobs
    try:
        utilMod.killScriptJobs(manipOrientScriptJobs)
    except NameError:
        manipOrientScriptJobs = [] 
    #scriptJob
    cmds.iconTextButton("manipOrientButton", edit=True, visible=True)
    
    #scriptJob
    manipOrientScriptJobs = []       
    jobString               = "updateManipOrient()"
    manipOrientScriptJobs.append(cmds.scriptJob(runOnce = False, killWithScene = False, event =('ToolChanged', jobString )))
    
    updateManipOrient()
    

  
def updateManipOrient():
    rot = cmds.manipRotateContext('Rotate', query=True, mode=True) 
    currentCtx  = cmds.currentCtx()   
    ctxTitle    = cmds.contextInfo(currentCtx, title=True)
    print "ctx", currentCtx, ctxTitle
    
    manipLabel = rot
    
    defaultWidth   = 25
    
    cmds.iconTextButton("manipOrientButton", edit=True, label="%s"%manipLabel)
    cmds.iconTextButton("manipOrientButton", edit=True, w=defaultWidth)
