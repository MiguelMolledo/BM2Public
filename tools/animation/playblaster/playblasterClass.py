...
class playblaster(object): 
    ...
    def toggleUI(self):
        if cmds.workspaceControl('playblaster', exists=True, q=True):
            if cmds.workspaceControl('playblaster', vis=True, q=True):
                cmds.workspaceControl('playblaster', vis=False, e=True)
            else:
                self.restoreUI()
        else:
            self.show()
    ...
    def checkJob(self):
        ...
        for job in sesionJobs:
            if 'refreshPlayblasterWindow()' in job:
                ...
                break
        if not jobExist:
            cmds.scriptJob(e=('PreFileNewOrOpened', self.refreshPlayblasterWindow), permanent=True)
    ...
    def createUi(self):
        ...
        cmds.menuItem('playblasterScale_50', label='50', rb=False, c= partial(self.setPlayblasterValue, 'scale', 50))
        cmds.menuItem('playblasterScale_70', label='70', rb=False, c= partial(self.setPlayblasterValue, 'scale', 70))
        cmds.menuItem('playblasterScale_100', label='100', rb=False, c= partial(self.setPlayblasterValue, 'scale', 100))
        cmds.setParent('..', menu=True)

        cmds.menuItem(divider=True)
        cmds.menuItem('playblasterShowornamentsOption', label='Show ornaments', cb= self.playblasterValues['showOrnaments'], c=partial(self.toggleItemsValues,'showOrnaments'))
        cmds.menuItem('playblasterSoundOption', label='Sound activate', cb= self.playblasterValues['activateSound'], c=partial(self.toggleItemsValues, 'activateSound'))
        cmds.menuItem('playblasterOffscreenOption', label='offscreen',cb= self.playblasterValues['offscreen'], c=partial(self.toggleItemsValues, 'offscreen'))

        showMenu = cmds.menu('showMenu', label='Show', p=menuLayout, pmc=self.preOpenShowMenu)
        cmds.menuItem('curvesVis', label='nurbs curves', cb=self.staticValues['nurbsCurves'], c=self.toggleNurbsCurves)

        cmds.menuItem('locatorsVis', label='locators', cb=self.staticValues['locators'], c=self.toggleLocators)
        cmds.menuItem('motionTrailsVis', label='motion trails', cb=self.staticValues['motionTrails'], c=self.toggleMotionTrails)
        cmds.menuItem(divider=True, dl='lights')

        cmds.menuItem('lightsOnVis', label='lights on', cb=self.staticValues['lightsOn'], c=self.toggleLightsOn)
        cmds.menuItem('shadowsVis', label='shadows', cb=self.staticValues['shadows'], c=self.toggleShadows)
        cmds.menuItem(divider=True, dl='2D planes')

        cmds.menuItem('iPlaneVis', label='images planes', cb=self.staticValues['imagePlane'], c=self.toggleImagePlanes)
        cmds.menuItem('greaseVis', label='grease pencil', cb=self.staticValues['greasePencils'], c=self.toggleGreasePencils)
        cmds.menuItem(divider=True)

        cmds.menuItem('manipulatorsVis', label='manipulators', cb=self.staticValues['manipulators'], c=self.toggleManipulators)
        ...
        cmds.button(label='- P L A Y B L A S T -', p=bottomLayout, en=True, c=self.blast)
        ...
        cmds.radioButton('temporaryPlayblast', label='Temporary Playblast', sl=True, p=checkLayout,
                         cc=partial(self.setPlayblasterValue,'saveOption','temporaryPlayblast'))
        cmds.radioButton('custom', label='Save Playblast', sl=False, p=checkLayout,
                         cc=partial(self.setPlayblasterValue,'saveOption','custom'))
    ...
    def setPlayblasterValue(self, item, value,*args):
    ...
    def blast(self,*args):
        ...
        if not name and self.playblasterValues['saveOption'] != 'temporaryPlayblast':
            if self.playblasterValues['saveOption'] == 'version2Review':
                return
            print 'choose the path to save the video',
            return
    ...
    def cameraInitState(self):
        ...
        self.getIplaneInfo(self.playblasterValues['currentCamera'])
        ...
        if self.playblasterValues['currentIPlane']:
            ...
            if cmds.getAttr(self.playblasterValues['currentIPlane'] + '.type') != 2 or 'playblaster' not in self.playblasterValues['currentIPlane']:
    ...
    def toggleNurbsCurves(self,*args):
        staticValues = functions.jsonPlayblaster()
    ...
    def toggleManipulators(self,*args):
        staticValues = functions.jsonPlayblaster()
    ...
    def toggleImagePlanes(self,*args):
        staticValues = functions.jsonPlayblaster()
    ...
    def toggleGreasePencils(self,*args):
        staticValues = functions.jsonPlayblaster()
    ...
    def toggleMotionTrails(self,*args):
        staticValues = functions.jsonPlayblaster()
    ...
    def toggleAppereance(self,*args):
    ...
    def toggleItemsValues(self, menuItem, *args):
        menuItemsCorrespondence = {'showOrnaments':'playblasterShowornamentsOption','activateSound':'playblasterSoundOption','offscreen':'playblasterOffscreenOption'}
        if cmds.menuItem(menuItemsCorrespondence[menuItem], q=True, cb=True):
            self.setPlayblasterValue(menuItem, False)
        else:
            self.setPlayblasterValue(menuItem, True)
